...
export async function version(ctx: HandlerContext): Promise<void> {
  ctx.emit({ name: 'netrunner', version: '0.7.9' }, ctx.human)
  process.exit(0)
}
...
export async function help(ctx: HandlerContext): Promise<void> {
  ...
  ctx.emit({
    ...
    commands: ['init', 'status', 'map', 'depth', 'explore', 'path', 'god-nodes', 'graph-report', 'plan', 'guard', 'persist', 'rollback', 'snapshot', 'policy', 'curate', 'lint', 'daemon', 'mesh', 'dump', 'install', 'uninstall', 'plugin', 'breach', 'deck', 'mode', 'quickhacks', 'resume', 'sleeve', 'doctor', 'history', 'mcp-orchestrate', '--mcp', '--acp', '--a2a'],
  }, ctx.human)
  process.exit(0)
}
