...
- 2026-08-26: **Deuda M1 pagada** (auditoría externa): `parse.ts` dividido en `parse-specs.ts` (config gramáticas) + `parse-imports.ts` (extractor por idioma) — parse.ts 306→168 líneas. `graph.ts:indexProject` extrajo `applyParsed` (71→~30). `queries.ts:impact` extrajo `expandBfs` (37→9). `mcp-server.ts:registerToolsetTools` extrajo `registerGraphTools`+`registerStackTool` (43→11 dispatcher). Todos los módulos <200 líneas. 42 tests verdes.
